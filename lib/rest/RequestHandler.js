                    ...
                    _respStream.on("data", (str) => {
                        ...
                        if(resp.headers["x-ratelimit-limit"]) {
                            ...
                            if (resp.headers["x-ratelimit-global"]) {
                                this._client.emit("warn", new Error(`Global ratelimit ${resp.headers["x-ratelimit-bucket"]}`));
                            } else {
                                this._client.emit("warn", new Error(`Ratelimit ${url} (${resp.headers["x-ratelimit-remaining"]}/${resp.headers["x-ratelimit-limit"]})`));
                            }
                        }
