                    ...
                    _respStream.on("data", (str) => {
                        ...
                        if(resp.headers["retry-after"]) {
                            if (resp.headers["x-ratelimit-global"]) {
                                this._client.emit("error", new Error(`Global ratelimit ${resp.headers["x-ratelimit-bucket"]}`));
                            } else {
                                this._client.emit("error", new Error(`Ratelimit ${url} ${resp.headers["x-ratelimit-bucket"]} (${resp.headers["x-ratelimit-reset-after"]})`));
                            }
                            if(resp.headers["x-ratelimit-global"]) {
                                this.globalBlock = true;
